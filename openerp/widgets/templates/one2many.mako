                ...
                <tr>
                    ...
                    <td>
                        ...
                        <button type="button" id="${name}_btn_" title="${new_attrs['help']}" onclick="new One2Many('${name}', ${(screen.view_type == 'tree' or 0) and len(screen.widget.editors)}).create()" style="padding: 2px">
                            % if parent_id:
                            <img src="${py.url('/static/images/stock/gtk-new.png')}" width="16" height="16"/>
                            % endif
                            % if not parent_id:
                            <img src="${py.url('/static/images/stock/gtk-save.png')}" width="16" height="16"/>
                            % endif
                        </button>
                        % endif
                        % if pager_info:
                        <button type="button" title="${_('Previous record...')}" onclick="submit_form('previous', '${name}')" style="padding: 2px">
                            <img class="button" src="${py.url('/static/images/stock/gtk-go-back.png')}" width="16" height="16"/>
                        </button>
                        % endif
                    </td>
                    % if pager_info:
                    ...
                    <td>
                        ...
                        <button type="button" title="${_('Next record...')}" onclick="submit_form('next', '${name}')" style="padding: 2px">
                            <img src="${py.url('/static/images/stock/gtk-go-forward.png')}" width="16" height="16"/>
                        </button>
                        % endif
                        <button type="button" title="${_('Switch view...')}" onclick="switch_O2M('${switch_to}', '${name}')" style="padding: 2px">
                            <img src="${py.url('/static/images/stock/gtk-justify-fill.png')}" width="16" height="16"/>
                        </button>
                    </td>
                    <td>
                        % if not screen.editable and screen.view_type=='form':
                        <img class="button" title="${_('Translate me.')}" src="${py.url('/static/images/stock/stock_translate.png')}" width="16" height="16" onclick="openobject.tools.openWindow('${py.url('/translator', _terp_model=screen.model, _terp_id=screen.id)}')"/>
                        % endif
                    </td>
                </tr>
            </table>

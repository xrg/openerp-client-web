<div class="pager">
    ...
    <table id="_${name+str(pager_id)}_limit_span" style="width: 100%; display: none" border="0" cellpadding="0" cellspacing="0">
        <tr>
            ...
            <td width="45px;">
                <select id='_${name+str(pager_id)}_limit' onchange="$('${name and (name != '_terp_list' or None) and name + '/'}_terp_limit').value=$('_${name+str(pager_id)}_limit').value;
                $('${name and (name != '_terp_list' or None) and name + '/'}_terp_offset').value= 0; pager_action('filter', '${name}')">
                    ...
                    %endfor
                    <option value="-1" ${py.selector(limit==-1)}>All</option>
                </select>
            </td>
        </tr>
    </table>

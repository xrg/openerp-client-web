...
<tr>
    <td width="100%" id="calNavigation">
        <table width="100%" class="toolbar">
            <tr>
                <td nowrap="nowrap">
                    <img height="16" width="16" class="button" src="${cp.static('base', 'images/stock/gtk-go-back.png')}" 
                ...
                <td nowrap="nowrap">
                    <img height="16" width="16" class="button" src="${cp.static('base', 'images/stock/gtk-go-forward.png')}" 
                        onclick="getCalendar('${days[-1].next().isoformat()}', null)"/>
                </td>
                <td nowrap="nowrap" width="100%"><strong>${title}</strong></td>
                <td nowrap="nowrap">
                    % if mode == 'day':
                    <img title="${_('Zoom In')}" height="16" width="16" src="${cp.static('base', 'images/stock-disabled/gtk-zoom-in.png')}"/>
                    % endif
                    % if mode != 'day':
                    <img title="${_('Zoom In')}" height="16" width="16" class="button" src="${cp.static('base', 'images/stock/gtk-zoom-in.png')}"
                        onclick="ganttZoomIn()"/>
                    % endif
                    
                    % if mode == '5years':
                    <img title="${_('Zoom Out')}" height="16" width="16" src="${cp.static('base', 'images/stock-disabled/gtk-zoom-out.png')}"/>
                    % endif
                    % if mode != '5years':
                    <img title="${_('Zoom Out')}" height="16" width="16" class="button" src="${cp.static('base', 'images/stock/gtk-zoom-out.png')}"
